...
class ClearDayDetection:
    ...
    def filter_for_sparsity(self, data: npt.NDArray[np.float64], w1=6e3, solver="OSQP"):
        ...
        self.filtered_signal = tl1_l2d2p365(
            self.density_signal, w1=w1, use_ixs=use_days, tau=0.85, solver=solver
        )
    ...
    def find_clear_days(
        ...
        self.y = tl1_l2d2p365(
            self.tc, tau=0.9, w1=2.5e5, yearly_periodic=False, solver=solver
        )
        tc = self.tc / self.y
        ...
        self.x = tl1_l2d2p365(
            self.de, tau=0.9, w1=2e5, yearly_periodic=False, solver=solver
        )
