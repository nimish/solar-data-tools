...
class PointBrowser(object):
    ...
    def submit(self, text):
        ...
        logger.info("selected index: " + str(self.lastind))
        self.update()

    def clicked(self, event):
        if self.lastind is None:
            logger.info("button click: nothing selected!")
            return
        sysid = self.data.iloc[self.lastind].name
        logger.info("button click: current ID: {}".format(sysid))
        ...
        if fn in cached_files:
            ...
            for cntr in range(CONFIG1["max_iteration"]):
                ...
                logger.info("min iteration {} complete".format(cntr + 1))
                obj_vals = iterative_fitting.calculate_objective_with_result(False)
        ...
        self.ax[4].legend()
        logger.info("first plot complete")
        with sns.axes_style("white"):
            self.ax[3].cla()
        ...
        self.ax[3].set_yticks([])
        self.ax[3].set_ylabel("(sunset)        Time of day        (sunrise)")
        logger.info("second plot complete")
        plt.tight_layout()
        self.fig.canvas.draw()
        return
    ...
    def onpress(self, event):
        ...
        logger.info("press event: " + str(event.key))
    ...
    def update(self):
        ...
        logger.info("updating, ID = {}".format(self.data.iloc[dataind].name))
