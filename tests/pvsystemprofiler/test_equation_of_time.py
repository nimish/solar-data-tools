...
@pytest.fixture
def eot_data_files(fixtures_dir: Path) -> dict[str, npt.NDArray[np.float64]]:
...
def test_eot_duffie(eot_data_files: dict[str, npt.NDArray[np.float64]]):
...
def test_eot_da_rosa(eot_data_files: dict[str, npt.NDArray[np.float64]]):
